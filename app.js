...
// ✅ PROBLEMA RESUELTO: Cursores de navegación visibles
// Solución aplicada: css/advanced-slider.css línea 103
...
function navegarSliderIndividual(sliderPadre, direccion) {
    ...
    // ✅ ENCONTRAR LA IMAGEN ACTUAL (la más visible)
    let imagenActual = imagenesVisibles.find(img => img.style.opacity === '1' || img.style.display !== 'none');
    ...
    // ✅ VERIFICAR LÍMITES
    if (nuevoIndice < 0) {
        console.log('🛑 Ya estás en la primera imagen del slider');
        return;
    }
    
    if (nuevoIndice >= imagenesVisibles.length) {
        console.log('🛑 Ya estás en la última imagen del slider');
        return;
    }
    
    // ✅ RESETEAR TODAS LAS IMÁGENES DEL SLIDER
    ...
    // ✅ RESALTAR LA IMAGEN ACTIVA CON EFECTO VISUAL
    const imagenNueva = imagenesVisibles[nuevoIndice];
    imagenNueva.style.opacity = '1';
    imagenNueva.style.filter = 'grayscale(0%)';
    imagenNueva.style.transform = 'scale(1)';
    imagenNueva.style.zIndex = '100';
    imagenNueva.style.boxShadow = '0 0 20px 5px #4CAF50';
    ...
    // ✅ CONFIRMACIÓN
    const direccionTexto = direccion === -1 ? 'anterior' : 'siguiente';
    console.log(`✅ Slider: imagen ${nuevoIndice + 1} de ${imagenesVisibles.length} (${direccionTexto})`);
    
    // ✅ RE-CREAR CURSORES SOLO PARA ESTE SLIDER
    setTimeout(() => {
        reCrearCursoresSlider(sliderPadre);
    }, 100);
}
...
function crearIndicadorNavegacionSlider(numeroActual, total, sliderPadre) {
    // Remover indicador anterior si existe
    const indicadorAnterior = document.getElementById(`indicador-slider-${sliderPadre.id || 'default'}`);
    if (indicadorAnterior) {
        indicadorAnterior.remove();
    }
    ...
    const indicador = document.createElement('div');
    indicador.id = `indicador-slider-${sliderPadre.id || 'default'}`;
    indicador.innerHTML = `
        <div style="
            position: absolute;
            top: 10px;
...
window.debugCursores = function() {
    console.log('\n=== DEBUG: ESTADO DE CURSORES ===');
    const imgs = document.querySelectorAll('img');
    const cursors = document.querySelectorAll('.cursor-nav-btn');
    
    console.log(`🖼️ Imágenes encontradas: ${imgs.length}`);
    console.log(`🖱️ Cursores encontrados: ${cursors.length}`);
    
    imgs.forEach((img, i) => {
        console.log(`\n--- IMAGEN ${i+1} ---`);
        console.log(`Dimensiones: ${img.offsetWidth}x${img.offsetHeight}px`);
        console.log(`Src: ${img.src ? 'SÍ' : 'NO'}`);
        console.log(`Parent: ${img.parentNode.tagName}`);
        console.log(`Califica para cursores: ${(img.offsetWidth >= 30 && img.offsetHeight >= 30) ? 'SÍ' : 'NO'}`);
    });
