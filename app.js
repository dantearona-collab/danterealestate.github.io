...
// Función principal para obtener información del entorno (DESDE JSON ESTÁTICO con fallback a API)
async function loadEnvironmentInfo(direccion, barrio) {
    ...
    try {
        ...
        // ========================================
        // PASO 1: Intentar cargar desde archivo JSON local
        // ========================================
        console.log('📁 Intentando cargar desde entorno.json local...');
        const environmentData = await loadEnvironmentFromJSON(barrio, direccion);
        
        // Verificar si tenemos suficientes categorías del archivo local
        const categoriasLocal = Object.keys(environmentData.categories || {}).length;
        console.log(`📊 Categorías del JSON local: ${categoriasLocal}`);
        
        // Si tenemos menos de 9 categorías, intentar fallback a API
        if (categoriasLocal < 9) {
            console.log('⚠️ Pocas categorías del JSON local, intentando fallback a API externa...');
            
            try {
                const apiData = await loadEnvironmentFromAPI(barrio);
                if (apiData) {
                    console.log('✅ Fallback a API externa exitoso');
                    // Mostrar en consola la fuente de datos
                    console.log('═══════════════════════════════════════════════');
                    console.log('📊 FUENTE DE DATOS: API EXTERNA (FALLBACK)');
                    console.log(`🏢 Barrio: ${barrio}`);
                    console.log(`📅 Fecha: ${new Date().toLocaleString()}`);
                    console.log('═══════════════════════════════════════════════');
                    
                    displayEnvironmentInfo(apiData);
                    return;
                }
            } catch (apiError) {
                console.warn('⚠️ Fallback a API externa falló:', apiError.message);
            }
        }
        
        // Usar datos del archivo local
        // Mostrar en consola la fuente de datos
        console.log('═══════════════════════════════════════════════');
        ...
        console.error('Error cargando entorno desde JSON local:', error);
        console.log('⚠️ Intentando fallback a API externa...');
        ...
        try {
            ...
            console.error('Error en fallback a API:', apiError);
        }
...
// Cargar datos del entorno desde API externa como fallback
async function loadEnvironmentFromAPI(barrio) {
    console.log('🌐 Consultando API externa para:', barrio);
    
    const barrioEncoded = encodeURIComponent(barrio);
    
    try {
        // Intentar con API de barrios
        const response = await fetch(`/api/barrios/${barrioEncoded}`);
        
        if (response.ok) {
            const result = await response.json();
            console.log('✅ Respuesta de API externa recibida');
            
            if (result.success && result.data) {
                // Transformar datos del formato backend al formato frontend
                const transformedData = transformBackendToRubros(result.data, barrio);
        ...
        console.warn('⚠️ Error consultando API de barrios:', error.message);
        
        // Intentar con la API externa onrender como último fallback
        try {
            console.log('🔄 Intentando con API externa alternativa...');
            const fallbackResponse = await fetch(`https://danterealestate-github-io-ewlg.onrender.com/api/barrios/${barrioEncoded}`);
            
            if (fallbackResponse.ok) {
                const fallbackResult = await fallbackResponse.json();
                console.log('✅ Respuesta de API alternativa recibida');
                
                if (fallbackResult.success && fallbackResult.data) {
                    const transformedData = transformBackendToRubros(fallbackResult.data, barrio);
                    return transformedData;
                }
            }
        } catch (fallbackError) {
            console.warn('⚠️ API alternativa también falló:', fallbackError.message);
        }
...
async function loadEnvironmentFromJSON(barrio, direccion) {
    console.log('🔍 loadEnvironmentFromJSON INICIADO para:', barrio);
    
    // Normalizar nombre del barrio para buscar en el JSON
    const barrioNormalizado = normalizeBarrioName(barrio);
    console.log('🔍 Barrio normalizado:', barrioNormalizado);
    
    // Cargar JSON si no está en cache
    if (!entornoCache) {
        try {
            // Intentar cargar desde user_input_files primero
            console.log('🔄 Cargando entorno.json desde user_input_files...');
            let response = await fetch('user_input_files/entorno.json');
            console.log('📡 Response status (user_input_files):', response.status);
            
            if (!response.ok) {
                // Fallback al root
                console.log('🔄 Intentando cargar desde /entorno.json...');
                response = await fetch('/entorno.json');
                console.log('📡 Response status (root):', response.status);
            }
            
            if (!response.ok) {
                throw new Error(`No se pudo cargar entorno.json: ${response.status}`);
            }
            
            entornoCache = await response.json();
            console.log('✅ Archivo entorno.json cargado correctamente');
            console.log('📊 Barrios disponibles:', Object.keys(entornoCache).slice(0, 5).join(', ') + '...');
        } catch (error) {
            console.error('❌ Error cargando entorno.json:', error);
            console.warn('⚠️ Usando datos por defecto');
            return createDefaultEnvironmentData(barrio, direccion);
        }
    } else {
        console.log('✅ Usando cache de entorno.json');
    }
    
    // Buscar datos del barrio específico
    console.log('🔍 Buscando datos para:', barrioNormalizado);
    ...
    console.log('✅ Datos encontrados para:', barrioData.nombre || barrio);
    
    // Usar la nueva función de transformación que maneja la estructura flat del entorno.json
    const result = transformEntornoJsonToDisplay(barrioData, barrio);
    console.log('✅ Transformación completada. Categorías:', Object.keys(result.categories).join(', '));
...
// Transformar datos del archivo entorno.json al formato de visualización
// Maneja la estructura flat con campos: nombre, resumen, conclusion, puntuacion_general, categorias (objeto con subcategorías)
// También兼容兼容 con la estructura anterior donde las categorías son campos a nivel raíz
function transformEntornoJsonToDisplay(data, barrio) {
    console.log('🔧 TRANSFORMANDO DATOS DE entorno.json:', data);
    console.log('📍 Barrio:', barrio);
    
    const categories = {};
    
    // Definir mapeo de categorías del entorno.json a títulos para display
    const categoryTitleMap = {
        ...
        'contaminacion': 'Contaminación y Ruidos',
        'vida_barrio': 'Vida de Barrio',
    ...
    // Procesar cada categoría en el objeto categorias (estructura nueva)
    if (data.categorias && typeof data.categorias === 'object') {
        Object.entries(data.categorias).forEach(([catKey, catData]) => {
            const title = categoryTitleMap[catKey] || catKey.charAt(0).toUpperCase() + catKey.slice(1).replace('_', ' ');
            const icon = categoryIconMap[catKey] || '📍';
            ...
            // Extraer descripción de la categoría
            let descripcion = '';
            if (typeof catData === 'object') {
                descripcion = catData.descripcion || '';
                // Si hay accesibilidad u otros datos, incorporarlos
                if (catData.accesibilidad) {
                    const accData = catData.accesibilidad;
                    if (typeof accData === 'object') {
                        Object.entries(accData).forEach(([key, value]) => {
                            descripcion += ` ${key.replace(/_/g, ' ')}: ${value}.`;
                        });
            ...
            // Crear item único para esta categoría con su descripción
            if (descripcion) {
                const puntuacion = (typeof catData === 'object' && catData.puntuacion) ? catData.puntuacion : null;
                
                categories[title].items.push({
                    name: title,
    ...
    // Si no hay objeto categorias, procesar campos a nivel raíz (estructura anterior/con híbrida)
    if (Object.keys(categories).length === 0) {
        const categoryKeys = Object.keys(categoryTitleMap);
        
        categoryKeys.forEach(catKey => {
            if (data[catKey]) {
                const title = categoryTitleMap[catKey];
                const icon = categoryIconMap[catKey];
                ...
                // Extraer contenido del campo
                const fieldData = data[catKey];
                let descripcion = '';
                
                if (Array.isArray(fieldData) && fieldData.length > 0) {
                    // Si es un array, tomar el primer elemento como descripción
                    descripcion = fieldData[0];
                } else if (typeof fieldData === 'string') {
                    descripcion = fieldData;
                }
    ...
    let descripcion = data.resumen || data.descripcion_general || '';
    
    // Si hay conclusion, agregarla
    if (data.conclusion) {
        descripcion += ' ' + data.conclusion;
    }
    
    // Generar metadata
    const metadata = {
        barrio: data.nombre || barrio,
        timestamp: data.fecha_actualizacion || new Date().toISOString(),
        version: '1.0',
        generadoPorIA: data.generado_por_ia || false
    };
    
    // Contar rubros total (cada categoría cuenta como 1 rubro en este formato)
    const totalRubros = Object.keys(categories).length;
    ...
    return {
        lastUpdated: new Date(metadata.timestamp).toLocaleString('es-AR'),
        descripcion: descripcion,
        barrio: metadata.barrio,
        categories: categories,
        metadata: metadata,
        totalRubros: totalRubros
    };
